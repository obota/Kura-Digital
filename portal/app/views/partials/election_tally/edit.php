                            ...
                            <div>
                                        ...
                                        <div class="col-sm-8">
                                            <div class="">
                                                <select required=""  id="ctrl-constituency" data-load-path="<?php print_link('api/json/election_tally_constituency_option_list') ?>" data-load-select-options="polling_center" name="constituency"  placeholder="Select a value ..."    class="selectize" >
                                                    <?php
                                                    $rec = $data['constituency'];
                                                    $constituency_options = $comp_model -> election_tally_constituency_option_list($data['county']);
                                                    if(!empty($constituency_options)){
                                                    foreach($constituency_options as $option){
                                        ...
                                        <div class="col-sm-8">
                                            <div class="">
                                                <select required=""  id="ctrl-polling_center" data-load-path="<?php print_link('api/json/election_tally_polling_center_option_list') ?>" data-load-select-options="polling_station" name="polling_center"  placeholder="Select a value ..."    class="selectize" >
                                                    <?php
                                                    $rec = $data['polling_center'];
                                                    $polling_center_options = $comp_model -> election_tally_polling_center_option_list($data['constituency']);
                                                    if(!empty($polling_center_options)){
                                                    foreach($polling_center_options as $option){
                                                    $value = (!empty($option['value']) ? $option['value'] : null);
                                                    $label = (!empty($option['label']) ? $option['label'] : $value);
                                                    $selected = ( $value == $rec ? 'selected' : null );
                                                    ?>
                                        ...
                                        <div class="col-sm-8">
                                            <div class="">
                                                <select required=""  id="ctrl-polling_station" data-load-path="<?php print_link('api/json/election_tally_polling_station_option_list') ?>" name="polling_station"  placeholder="Select a value ..."    class="selectize" >
                                                    <?php
                                                    $rec = $data['polling_station'];
                                                    $polling_station_options = $comp_model -> election_tally_polling_station_option_list($data['polling_center']);
                                                    if(!empty($polling_station_options)){
                                                    foreach($polling_station_options as $option){
                                                    $value = (!empty($option['value']) ? $option['value'] : null);
                                                    $label = (!empty($option['label']) ? $option['label'] : $value);
                                                    $selected = ( $value == $rec ? 'selected' : null );
                                                    ?>
                                ...
                                <div class="form-group ">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-8">
                                            <div class="">
                                                <input id="ctrl-votes"  value="<?php  echo $data['votes']; ?>" type="number" placeholder="Enter Garnered Votes" step="1"  required="" name="votes"  class="form-control " />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
